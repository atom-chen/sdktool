...
def mergeManifest(channel, targetManifest, sdkManifest):
    ...
    if appConfigNode != None:
        ...
        if proxyApplicationName != None and len(proxyApplicationName) > 0:
            ...
            if 'YX_APPLICATION_PROXY_NAME' in channel:
                
                channel['YX_APPLICATION_PROXY_NAME'] = channel['YX_APPLICATION_PROXY_NAME'] + ',' + proxyApplicationName
            else:
                
                channel['YX_APPLICATION_PROXY_NAME'] = proxyApplicationName
...
def copySplashToUnityResFolder(workDir, channel, decompileDir):
    ...
    resPath = workDir + "/sdk/" + channel['sdk'] + "/splash/" + channel['splash'] + "/%s/yxme_splash.png"
    ...
    if not bFound:
        log_utils.error("the yxme_splash is not found.path:%s", resPath)
        return 1
...
def addSplashScreen(workDir, channel, decompileDir):
    ...
    splashLayoutPath = splashPath + "/yx_splash.xml"
    splashTargetPath = decompileDir + "/res/layout/yx_splash.xml"
    ...
    splashActivityPath = smaliTargetPath + "/com/yxme/sdk/SplashActivity.smali"
    f = open(splashActivityPath, 'r+')
    content = str(f.read())
    f.close()

    replaceTxt = '{YxmeSDK_Game_Activity}'

    idx = content.find(replaceTxt)
    if idx == -1:
        log_utils.error("modify splash file failed.the {YxmeSDK_Game_Activity} not found in SplashActivity.smali")
        return 1
...
def appendSplashActivity(decompileDir, splashType):
    ...
    splashNode.set(key, 'com.yxme.sdk.SplashActivity')
...
def writeDevelopInfo(game, channel, decompileDir):
    ...
    developConfigFile = os.path.join(developConfigFile, "yx_developer_config.properties")
...
def writePluginInfo(channel, decompileDir):
    ...
    developConfigFile = os.path.join(developConfigFile, "yx_plugin_config.xml")
...
def writeManifestMetaInfo(channel, decompileDir):
    ...
    if 'YX_APPLICATION_PROXY_NAME' in channel:
        metaNode = SubElement(appNode, 'meta-data')
        metaNode.set(key, "YX_APPLICATION_PROXY_NAME")
        metaNode.set(val, channel['YX_APPLICATION_PROXY_NAME'])        
...
def checkApkForU8SDK(workDir, decompileDir):
    ...
    """
    ret = 0
    log_utils.info("now to check the yxme.apk is correct?")
    ...
    if not name or name != "com.yxme.sdk.YxmeApplication":
        log_utils.error("the android:name in application element must be 'com.yxme.sdk.YxmeApplication'. now change it to com.yxme.sdk.YxmeApplication, but maybe something will be wrong .")
        applicationNode.set(key, 'com.yxme.sdk.YxmeApplication')
        tree.write(manifestFile, 'UTF-8')

    smaliName = file_utils.getFullPath(decompileDir + "/smali/com/yxme/sdk/YxmeSDK.smali")
    if not os.path.exists(smaliName):
        log_utils.error("the portal-sdk.jar is not packaged to the yxme.apk. now merge it. but maybe something will be wrong .")

        u8sdkJarPath = file_utils.getFullPath('config/local/portal-sdk.jar')
        if not os.path.exists(u8sdkJarPath):
            log_utils.error("the portal-sdk.jar is not in config/local path. check failed")
            return 1
        ...
        file_utils.copy_file(u8sdkJarPath, targetPath+"/portal-sdk.jar")
    ...
    log_utils.info("check yxme.apk successfully")
...
def splitDex(workDir, decompileDir):
    ...
    if not os.path.exists(multidexFilePath):
        ...
        if not os.path.exists(dexJar):
            log_utils.error("the method num expired of dex, but no android-support-multidex.jar in yxme.apk or in local folder")
            return
    ...
    for f in allFiles:
        f = f.replace("\\", "/")
        if "/com/yxme/sdk" in f or "/android/support/multidex" in f:
